...
class Solution {
	static int order[][]={{0,1,2},{0,2,1},{1,0,2},{1,2,0},{2,0,1},{2,1,0}};//모든 우선순위 경우의 수
	static char c[]= {'+','-','*'};
    public long solution(String expression) {
		...
		for(int i=0;i<expression.length();i++) {
            ...
            //StringBuilder를 이용하여 숫자와 연산자를 분리
	...
	static long calc(ArrayList<String> arr, int n) {
		ArrayList<String> list=new ArrayList<>();
            ...
            //우선순위에 맞게 수식에서 계산
            for(int j=0;j<3;j++) {
			    char operator=c[order[n][j]];// c -> 연산자  order[][] -> 우선순위
                
			    for(int k=1;k<list.size()-1;k++) {
				    String cs=list.get(k);
                    //우선순위에 맞는 연산자일 경우 연산자 앞, 뒤의 값을 계산
